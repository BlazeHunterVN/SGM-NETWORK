...
async function initSupabase() {
    if (typeof createClient === 'undefined') return;
    try {
        ...
        if (config.supabaseUrl && config.supabaseAnonKey) {
            ...
            supabaseClient.channel('public:nation_banners').on('postgres_changes', { event: '*', schema: 'public', table: 'nation_banners' }, (payload) => {
                console.log('Realtime update detected (Banners):', payload);
                fetchDataFromAPI();
            }).subscribe();
            supabaseClient.channel('public:home_settings').on('postgres_changes', { event: '*', schema: 'public', table: 'home_settings' }, (payload) => {
...
async function callApi(action, params = {}) {
    try {
        const response = await fetch('/api/admin', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ action, params }) });
...
const translations = {
    'vietnam': { 'home_link': 'TRANG CHỦ', 'nation_link': 'QUỐC GIA', 'news_link': 'TIN TỨC', 'contact_link': 'LIÊN HỆ', 'select_country': 'QUỐC GIA ĐANG CHỌN', 'news_heading': 'TIN TỨC', 'contact_heading': 'LIÊN HỆ VỚI CHÚNG TÔI', 'contact_info': 'THÔNG TIN LIÊN HỆ Ở ĐÂY.', 'operating_policy': 'CHÍNH SÁCH VẬN HÀNH', 'terms_of_use': 'ĐIỀU KHOẢN SỬ DỤNG', 'select_prompt': 'VUI LÒNG CHỌN MỘT QUỐC GIA', 'update_banner': 'DỮ LIỆU BIỂU NGỮ ĐANG ĐƯỢC CẬP NHẬT.', 'update_news': 'DỮ LIỆU TIN TỨC ĐANG ĐƯỢC CẬP NHẬT.', 'start_date': 'NGÀY BẮT ĐẦU', 'date_posting': 'NGÀY ĐĂNG', 'link_access': 'TRUY CẬP LIÊN KẾT', 'no_title': 'KHÔNG CÓ TIÊU ĐỀ', 'active': 'ĐANG HOẠT ĐỘNG', 'upcoming': 'SẮP DIỄN RA', 'ending': 'SẮP KẾT THÚC', 'check_back': 'VUI LÒNG KIỂM TRA LẠI SAU.', 'check_back_news': 'VUI LÒNG KIỂM TRA LẠI SAU ĐỂ CẬP NHẬT TIN TỨC MỚI NHẤT.', 'download_image': 'TẢI ẢNH VỀ THIẾT BỊ' },
    'default': { 'home_link': 'HOME', 'nation_link': 'NATION', 'news_link': 'NEWS', 'contact_link': 'CONTACT', 'select_country': 'CURRENT NATION', 'news_heading': 'NEWS', 'contact_heading': 'CONTACT US', 'contact_info': 'CONTACT INFORMATION HERE.', 'operating_policy': 'OPERATING POLICY', 'terms_of_use': 'TERMS OF USE', 'select_prompt': 'PLEASE SELECT A COUNTRY', 'update_banner': 'BANNER DATA IS CURRENTLY BEING UPDATED.', 'update_news': 'NEWS DATA IS CURRENTLY BEING UPDATED.', 'start_date': 'START DATE', 'date_posting': 'DATE OF POSTING', 'link_access': 'LINK ACCESS', 'no_title': 'NO TITLE', 'active': 'ACTIVE', 'upcoming': 'UPCOMING', 'ending': 'ENDING', 'check_back': 'PLEASE CHECK BACK LATER.', 'check_back_news': 'PLEASE CHECK BACK LATER FOR THE LATEST NEWS.', 'download_image': 'DOWNLOAD IMAGE' }
};

let currentLanguage = localStorage.getItem('currentLang') || 'default';
let userInteracted = false;
const initialNationData = { brazil: { images: [] }, india: { images: [] }, indonesia: { images: [] }, pakistan: { images: [] }, singapore: { images: [] }, taiwan: { images: [] }, thailand: { images: [] }, vietnam: { images: [] }, news: { images: [] } };
...
function getIsMobile() { return window.matchMedia("(max-width: 768px)").matches; }
...
function lazyLoadImage(img) { if (imageObserver && img.dataset.src) imageObserver.observe(img); }
...
function observeScrollAnimation(element) { if (scrollAnimationObserver && element) scrollAnimationObserver.observe(element); }

function applyTranslation(langKey) {
    const translationSet = translations[langKey] || translations['default'];
    document.querySelectorAll('[data-i18n]').forEach(element => {
        const key = element.getAttribute('data-i18n');
        if (translationSet[key]) element.textContent = translationSet[key];
    });
    ...
    if (activeLangLinks) activeLangLinks.forEach(link => link.classList.add('active'));
    currentLanguage = langKey;
    const currentPath = window.location.pathname;
    const parts = currentPath.split('/');
    const key = parts[parts.length - 1];
    if (currentPath.startsWith('/nation') || currentPath === '/news') updateSectionHeadings(currentPath, key);
}
...
function changeLanguageAndReload(langKey) {
    if (langKey) {
        ...
        else if (currentPath === '/nation') displayImages('default', false);
...
async function fetchDataFromAPI() {
    try {
        ...
        rawData.forEach(item => {
            ...
            if (!nationData[nationKey]) nationData[nationKey] = { images: [] };
            nationData[nationKey].images.push({ url: item.url, startDate: item.start_date, bannerLink: item.banner_link, title: item.title, endDate: item.end_date, id: item.id });
        });
        const currentPath = window.location.pathname;
        const parts = currentPath.split('/');
        const key = parts[parts.length - 1];
        ...
        if (imageGrid) {
            imageGrid.innerHTML = `<div style="text-align: center; padding: 50px; color: #d32f2f;"><h2>⚠️ Connection Error</h2><p>Unable to load content. Please try again later.</p></div>`;
        }
    }
}
...
async function fetchHomeSettings() {
    ...
    if (cachedPc || cachedMobile) applyHomeBackgrounds({ bg_pc_url: cachedPc, bg_mobile_url: cachedMobile });
    try {
...
function applyHomeBackgrounds(settings) {
    ...
    if (pcUrl && pcUrl.trim() !== '') {
        ...
        if (pcUrl.match(/\.(mp4|webm|ogg)$/i)) {
            ...
            if (video) {
                video.src = pcUrl;
                if (userInteracted && section.classList.contains('active') && !getIsMobile()) video.play().catch(() => { });
            }
            if (pcImg) pcImg.style.display = 'none';
        } else {
            console.log("[Background-Engine] Applying PC IMAGE:", finalPcUrl);
            if (pcImg) { pcImg.src = finalPcUrl; pcImg.style.display = 'block'; }
    ...
    if (mobileUrl && mobileUrl.trim() !== '') {
        ...
        if (mobileImg) mobileImg.src = finalMobileUrl;
    }
}
...
function showSection(section) {
    ...
    if (section !== homeSection) stopVideo();
    else if (userInteracted) tryToPlayVideo();
    const body = document.body;
    if (section === homeSection) body.classList.remove('non-home-bg');
    else body.classList.add('non-home-bg');
    if (section === nationSection || section === contactSection || section === newsSection) body.classList.add('hide-footer');
    else body.classList.remove('hide-footer');
    window.scrollTo(0, 0);
}
...
function updateSectionHeadings(path, key) {
    ...
    if (path.startsWith('/nation/')) {
        ...
        if (window.innerWidth <= 768) nationHeading.textContent = `${t['select_country']} - ${t['select_prompt']}`;
        else nationHeading.textContent = '';
    } else if (path === '/news') {
...
function handleRouting(path, key) {
    ...
    allNavLinks.forEach(link => { if (!link.closest('.language-selector')) link.classList.remove('active'); });
    let activeNavPath = '/';
    if (path.startsWith('/nation')) activeNavPath = '/nation';
    else if (path.startsWith('/news')) activeNavPath = '/news';
    else if (path.startsWith('/contact')) activeNavPath = '/contact';
    ...
    if (path === '/') { showSection(homeSection); if (nationHeading) nationHeading.textContent = ''; }
    else if (path.startsWith('/nation/')) { showSection(nationSection); updateSectionHeadings(path, key); displayImages(key, false); }
    else if (path === '/nation') { showSection(nationSection); updateSectionHeadings(path, key); displayImages('default', false); }
    else if (path === '/news') { showSection(newsSection); updateSectionHeadings(path, key); displayImages('news', true); }
    else if (path === '/contact') showSection(contactSection);
    else showSection(homeSection);
...
function isImageKitUrl(url) { return url && typeof url === 'string' && url.includes(IK_URL_ENDPOINT); }

function getEventStatus(startDateString) {
    const t = translations[currentLanguage];
    if (!startDateString) return { status: 'none', label: '' };
    const parts = startDateString.split('/');
    if (parts.length !== 3) return { status: 'none', label: '' };
    const [day, month, year] = parts.map(Number);
    const startDate = new Date(Date.UTC(year, month - 1, day));
    if (isNaN(startDate.getTime())) return { status: 'none', label: '' };
    const now = new Date();
    const today = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate()));
    const eventDay = new Date(startDate.getUTCFullYear(), startDate.getUTCMonth(), startDate.getUTCDate());
    if (eventDay <= today) return { status: 'active', label: t['active'] };
    else return { status: 'upcoming', label: t['upcoming'] };
}
...
function getBannerStatus(startDateStr, endDateStr) {
    ...
    let end;
    if (endDateStr && endDateStr.trim() !== '') end = convertDateStringToDate(endDateStr);
    else { end = new Date(start.getTime()); end.setUTCDate(end.getUTCDate() + 10); }
    const now = new Date();
    const today = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate()));
    start.setUTCHours(0, 0, 0, 0);
    end.setUTCHours(0, 0, 0, 0);
    if (today < start) return { status: 'upcoming', label: t['upcoming'] || 'UPCOMING', endDate: end };
    ...
    if (diffDaysSinceEnd >= 0) {
        if (diffDaysSinceEnd > 30) return { status: 'none', label: '', endDate: end };
...
function convertDateStringToDate(dateString) {
    ...
    if (parts.length === 3) {
        let day, month, year;
        if (parts[0].length === 4) { year = parseInt(parts[0], 10); month = parseInt(parts[1], 10); day = parseInt(parts[2], 10); }
    ...
    if (!isNaN(parsed.getTime())) return new Date(Date.UTC(parsed.getUTCFullYear(), parsed.getUTCMonth(), parsed.getUTCDate()));
    return new Date(0);
}

function getOptimizedImageAttributes(imageDataUrl, altText) {
    const defaultAttributes = { src: imageDataUrl || '', alt: altText };
    if (window.ImageKit && isImageKitUrl(imageDataUrl)) {
        const relativePath = imageDataUrl.replace(IK_URL_ENDPOINT, '');
        try {
            return window.ImageKit.getResponsiveImageAttributes({ urlEndpoint: IK_URL_ENDPOINT, src: relativePath, sizes: "(min-width: 1025px) 30vw, (min-width: 769px) 45vw, 90vw", transformation: [{ format: "auto", crop: "at_max" }], alt: altText });
        } catch (error) { console.error("ImageKit error:", error); return defaultAttributes; }
    }
...
function displayImages(key, isNews = false) {
    ...
    if (images.length === 0 || key === 'default' || !nationData[key]) {
        ...
        let message = '';
        if (key === 'default') message = `<h1>${t['select_prompt']}</h1><p>${t['update_banner'].replace('DỮ LIỆU BIỂU NGỮ', 'DỮ LIỆU BIỂU NGỮ')}</p>`;
        else if (isNews) message = `<h1>${t['update_news']}</h1><p>${t['check_back_news']}</p>`;
        else message = `<h1>${t['update_banner']}</h1><p>${t['check_back']}</p>`;
        targetGrid.innerHTML = `<div style="text-align: center; width: 90%; max-width: 400px; padding: 50px 20px; color: #2e2e2eff; box-sizing: border-box;">${message}</div>`;
        return;
    }
    targetGrid.removeAttribute('style');
    images = images.filter(imageData => {
        ...
        const today = new Date(Date.UTC(now.getUTCFullYear(), now.getUTCMonth(), now.getUTCDate()));
        let end;
        if (imageData.endDate && imageData.endDate.trim() !== '') end = convertDateStringToDate(imageData.endDate);
        else { end = new Date(start.getTime()); end.setUTCDate(end.getUTCDate() + 10); }
        end.setUTCHours(0, 0, 0, 0);
        const diffTimeSinceEnd = today.getTime() - end.getTime();
        const diffDaysSinceEnd = Math.floor(diffTimeSinceEnd / (1000 * 60 * 60 * 24));
...
function handleNavLinkClick(e) {
    ...
    if (isLanguageLink) { if (e.target.dataset.lang) return; if (window.innerWidth <= 768 && e.currentTarget.closest('#mobileLangSelector')) return; return; }
    if (window.innerWidth <= 768 && isNationParentLink) { if (nationDropdownLi) { nationDropdownLi.classList.toggle('active'); if (mobileLangSelector) mobileLangSelector.classList.remove('active'); } return; }
    if (isNationDropdownLink) { if (nationDropdownLi) nationDropdownLi.classList.remove('active'); }
    if (path.startsWith('http://') || path.startsWith('https://') || path === '#') { if (path.startsWith('http') || path.startsWith('https')) window.open(path, '_blank'); return; }
    const isMainPage = document.getElementById('home-section') !== null;
    if (!isMainPage) { window.location.href = path; return; }
...
if (logoLink) { logoLink.addEventListener('click', (e) => { e.preventDefault(); handleRouting('/', ''); }); }
languageLinks.forEach(link => {
    link.addEventListener('click', (e) => {
        e.preventDefault(); e.stopPropagation();
        const selectedLangKey = e.target.dataset.lang;
        if (selectedLangKey === 'vietnam' || selectedLangKey === 'default') changeLanguageAndReload(selectedLangKey);
    });
});
if (mobileLangSelector) {
    ...
    if (langToggleLink) {
        langToggleLink.addEventListener('click', (e) => {
            e.preventDefault();
            if (window.innerWidth <= 768) { e.stopPropagation(); mobileLangSelector.classList.toggle('active'); if (nationDropdownLi) nationDropdownLi.classList.remove('active'); }
        });
    }
}
            ...
            if (gridItem && !gridItem.classList.contains('updating-message')) {
                ...
                if (data) {
                    ...
                    if (gridOverlay) {
                        ...
                        let dateHTML = "";
                        if (startDate && startDate.trim() !== '') { const dateLabel = isNews ? t['date_posting'] : t['start_date']; dateHTML += `${dateLabel}: ${startDate}`; }
                        if (endDate && endDate.trim() !== '') { dateHTML += (dateHTML ? "<br>" : "") + `END DATE: ${endDate}`; }
                        if (dateHTML) { overlayDate.innerHTML = dateHTML; overlayDate.style.display = 'block'; }
                        else { overlayDate.innerHTML = ''; overlayDate.style.display = 'none'; }
                        overlayLink.textContent = t['link_access'];
                        if (data.bannerLink) { overlayLink.href = data.bannerLink; overlayLink.style.display = 'block'; if (overlayDisclaimer) overlayDisclaimer.style.display = 'block'; }
                        else { overlayLink.href = '#'; overlayLink.style.display = 'none'; if (overlayDisclaimer) overlayDisclaimer.style.display = 'none'; }
                        if (overlayDownload) overlayDownload.style.display = 'none';
...
if (closeBtn) { closeBtn.addEventListener('click', () => { if (gridOverlay) gridOverlay.style.display = 'none'; document.body.style.overflow = 'auto'; }); }
if (gridOverlay) { gridOverlay.addEventListener('click', (e) => { if (e.target === gridOverlay) { gridOverlay.style.display = 'none'; document.body.style.overflow = 'auto'; } }); }
...
if (menuToggle) {
    menuToggle.addEventListener('click', () => {
        ...
        const menuIcon = menuToggle.querySelector('i');
        if (!menuIcon) return;
        if (navbar.classList.contains('active')) { menuIcon.classList.remove('fa-bars'); menuIcon.classList.add('fa-times'); document.body.style.overflow = 'hidden'; }
        else { menuIcon.classList.remove('fa-times'); menuIcon.classList.add('fa-bars'); document.body.style.overflow = 'auto'; }
    });
}
document.addEventListener('click', function (e) {
    if (mobileLangSelector && window.innerWidth <= 768) {
        if (mobileLangSelector.classList.contains('active') && !mobileLangSelector.contains(e.target)) {
            if (!e.target.closest('.menu-toggle')) mobileLangSelector.classList.remove('active');
        }
    }
    if (window.innerWidth <= 768 && nationDropdownLi && nationDropdownLi.classList.contains('active')) {
        if (!navbar.contains(e.target) && !mobileLangSelector.contains(e.target)) nationDropdownLi.classList.remove('active');
    }
});
async function setupRealtimeSync() {
    if (typeof supabase === 'undefined') { console.warn("[Realtime] Supabase library not detected."); return; }
    const client = supabase.createClient(SUPABASE_URL, SUPABASE_KEY);
    client.channel('banner_sync').on('postgres_changes', { event: '*', schema: 'public', table: SUPABASE_TABLE }, async (payload) => {
    ...
    client.channel('settings_sync').on('postgres_changes', { event: '*', schema: 'public', table: 'home_settings' }, async () => {
...
window.addEventListener('load', () => {
    const loadingScreen = document.getElementById('loading-screen');
    if (loadingScreen) { setTimeout(() => { loadingScreen.classList.add('hidden'); }, 1500); }
});
document.addEventListener('click', tryToPlayVideo, { once: true });
document.addEventListener('touchend', tryToPlayVideo, { once: true });
document.addEventListener('contextmenu', e => { e.preventDefault() }); document.addEventListener('keydown', e => { if (e.key === 'F12' || (e.ctrlKey && e.shiftKey && (e.key === 'I' || e.key === 'J')) || (e.ctrlKey && e.key === 'U')) { e.preventDefault() } if (e.ctrlKey && (e.key === 'c' || e.key === 'v' || e.key === 's' || e.key === 'p')) { e.preventDefault() } }); document.addEventListener('dragstart', e => { if (e.target.tagName === 'IMG') { e.preventDefault() } });
