...
const translations = {
    'vietnam': {
        ...
        'update_esports': 'DỮ LIỆU ESPORTS ĐANG ĐƯỢC CẬP NHẬT.',
    ...
    'default': {
        ...
        'update_esports': 'ESPORTS DATA IS CURRENTLY BEING UPDATED.',
...
const esportsSection = document.getElementById('esports-section');
...
function applyTranslation(langKey) {
    ...
    if (currentPath.startsWith('/nation') || currentPath === '/news' || currentPath === '/esports') {
...
async function fetchDataFromAPI() {
    try {
        ...
        else if (currentPath === '/esports') displayImages('esports', true);
...
function updateSectionHeadings(path, key) {
    ...
    if (!nationHeading || !newsHeading || !esportsHeading) return;
    ...
    else if (path === '/esports') {
        esportsHeading.textContent = t['esports_heading'];
    }
}
...
function handleRouting(path, key) {
    ...
    if (path.startsWith('/nation')) {
        ...
        activeNavPath = '/news';
    } else if (path.startsWith('/esports')) {
        activeNavPath = '/esports';
...
function displayImages(key, isNews = false) {
    let targetGrid = imageGrid;
    if (key === 'news') targetGrid = newsImageGrid;
    if (key === 'esports') targetGrid = esportsImageGrid;
    ...
    if (images.length === 0 || key === 'default' || !nationData[key]) {
        ...
        } else if (isNews && key === 'news') {
            message = `<h1>${t['update_news']}</h1><p>${t['check_back_news']}</p>`;
        } else if (key === 'esports') {
            message = `<h1>${t['update_esports']}</h1><p>${t['check_back_esports']}</p>`;
        } else {
    ...
    images.forEach((imageData, index) => {
        ...
        const isStandardBanner = (!isNews && key !== 'esports'); // Logic to show badges: Only for standard banners
                ...
                if (data) {
                    ...
                    if (gridOverlay) {
                        ...
                        if (data.bannerLink) {
                            ...
                            if (overlayDisclaimer) overlayDisclaimer.style.display = 'block';
                        } else {
                            overlayLink.href = '#';
                            overlayLink.style.display = 'none';
                            if (overlayDisclaimer) overlayDisclaimer.style.display = 'none';
                        }
...
document.addEventListener('keydown', (e) => {
    ...
    // Prevent Ctrl+C, Ctrl+V, Ctrl+S, Ctrl+P
    if (e.ctrlKey && (e.key === 'c' || e.key === 'v' || e.key === 's' || e.key === 'p')) {
        e.preventDefault();
    }
});
