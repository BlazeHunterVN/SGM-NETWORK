...
async function initSupabase() {
    ...
    try {
        ...
        try {
            ...
            supabaseUrl = 'https://biedzzdbjdleuiejuyhi.supabase.co';
            supabaseAnonKey = 'eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.eyJpc3MiOiJzdXBhYmFzZSIsInJlZiI6ImJpZWR6emRiamRsZXVpZWp1eWhpIiwicm9sZSI6ImFub24iLCJpYXQiOjE3Njc0NTAzNjUsImV4cCI6MjA4MzAyNjM2NX0.Sc81H_kpxy4GFw1x9QHibkGkRgO469BZLlmCHQEpoGE';
        }
...
function playLoginAnimation(fileName, loop = true, callback = null) {
    ...
    // If same animation is already playing, skip re-loading unless loop changes
    if (currentAnimationFile === fileName && mainAnimation) {
        return Promise.resolve(mainAnimation);
    }
    ...
    return new Promise((resolve, reject) => {
        fetch(`/admin/assets/json/${fileName}`)
            ...
            .then(data => {
                // Check if another animation was requested while we were fetching
                if (currentAnimationFile !== fileName) {
                    resolve(null);
                    return;
                }
            ...
            .catch(err => {
                console.error(`Status: Failed to load animation ${fileName}:`, err);
                currentAnimationFile = null;
                reject(err);
            });
    });
}
    ...
    if (supabaseClient) {
        try {
            ...
            if (session && session.user) {
                ...
                const animPromise = playLoginAnimation('loading.json', true);
                showChecking('Verifying access...');

                // Whitelist check
                const adminData = await checkWhitelist(email);

                // Wait for at least 2 loops of loading animation to make it look smooth
                ...
                if (adminData) {
                    ...
                    // Retrieve the access_key from the API using the current JWT
                    try {
...
function showDashboard() {
    ...
    // First setup from local storage for fast render
    let role = localStorage.getItem('admin_role') || 'admin';
    if (grantTabLink) {
        grantTabLink.style.display = (role === 'senior_admin') ? 'flex' : 'none';
    }

    // Then asynchronously re-verify role to fix missing localStorage roles from older sessions
    const email = localStorage.getItem('admin_email');
    if (email && grantTabLink) {
        checkWhitelist(email).then(adminData => {
            if (adminData && adminData.role) {
                localStorage.setItem('admin_role', adminData.role);
                grantTabLink.style.display = (adminData.role === 'senior_admin') ? 'flex' : 'none';
                console.log('Re-verified role: ', adminData.role);
            }
        }).catch(err => console.error('Failed to re-verify admin role:', err));
    }
