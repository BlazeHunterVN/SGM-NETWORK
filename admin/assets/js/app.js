        ...
        if (config.supabaseUrl && config.supabaseAnonKey) {
            ...
            supabaseClient
                ...
                .on('postgres_changes', { event: '*', schema: 'public', table: 'nation_banners' }, (payload) => {
                    ...
                    if (payload.new && payload.new.nation_key === 'esports') {
                        fetchEsports();
                    }
                })
                .subscribe();
...
// const btnGoogleLogin = document.getElementById('btn-google-login'); // Removed
...
const btnSaveEsports = document.getElementById('btn-save-esports');
...
//     alert('Google Login is currently being secured. Please use Email/Key login for now.');
//     // In a full implementation, this would redirect to a server-side auth route
// }
...
function renderNews(news) {
    ...
    news.forEach(item => {
        ...
        tr.innerHTML = `
            ...
            <td><img src="${item.url}" alt="News" style="height: 50px; object-fit: cover;"></td>
...
// ============ ESPORTS MANAGEMENT ============
const esportsForm = document.getElementById('esports-form');
const esportsTableBody = document.getElementById('esports-table-body');
const filterEsportsFrom = document.getElementById('filter-esports-from');
const filterEsportsTo = document.getElementById('filter-esports-to');
const btnApplyEsportsFilter = document.getElementById('btn-apply-esports-filter');
const selectAllEsports = document.getElementById('select-all-esports');
const btnDeleteEsportsSelected = document.getElementById('btn-delete-esports-selected');

let allEsports = [];
let isEditingEsports = false;
let currentEditEsportsId = null;

async function fetchEsports() {
    try {
        const data = await callAdminApi('fetch_banners', { nation_key: 'esports' });
        allEsports = data;
        filterAndRenderEsports();
    } catch (error) {
        console.error('Error fetching esports:', error);
    }
}

function filterAndRenderEsports() {
    const dateFromStr = filterEsportsFrom ? filterEsportsFrom.value : '';
    const dateToStr = filterEsportsTo ? filterEsportsTo.value : '';

    let filtered = allEsports;
    ...
    if (dateFrom || dateTo) {
        filtered = filtered.filter(n => {
            const itemDate = parseDateString(n.start_date);
    ...
    renderEsports(filtered);
}

function renderEsports(items) {
    if (!esportsTableBody) return;
    esportsTableBody.innerHTML = '';
    items.forEach(item => {
        const tr = document.createElement('tr');
        tr.innerHTML = `
            <td><input type="checkbox" class="esports-checkbox" data-id="${item.id}"></td>
            <td><img src="${item.url}" alt="Esports" style="height: 50px; object-fit: cover;"></td>
            <td>${item.title || 'No Title'}</td>
            <td>${item.start_date || '--/--/----'}</td>
            <td>
                <button class="action-btn btn-edit" onclick="openEditEsportsModal('${item.id}', '${item.title || ''}', '${item.url}', '${item.banner_link || ''}', '${item.start_date || ''}')"><i class="fas fa-edit"></i></button>
                <button class="action-btn btn-delete" onclick="deleteEsports('${item.id}')"><i class="fas fa-trash"></i></button>
            </td>
        `;
        esportsTableBody.appendChild(tr);
    });

    updateEsportsCheckboxListeners();
}

window.deleteEsports = async (id) => {
    if (confirm('Are you sure you want to delete this esports news?')) {
        const email = localStorage.getItem('admin_email');
        const key = localStorage.getItem('admin_key');
...
window.openEditEsportsModal = (id, title, url, link, startDate) => {
    isEditingEsports = true;
    currentEditEsportsId = id;

    const formHeader = esportsForm.previousElementSibling.querySelector('h3');
    if (formHeader) formHeader.textContent = 'EDIT ESPORTS NEWS';

    document.getElementById('esports-title').value = title;
    document.getElementById('esports-url').value = url;
    document.getElementById('esports-link').value = link;
    document.getElementById('esports-posting-date').value = startDate;

    const saveBtn = document.getElementById('btn-save-esports');
    if (saveBtn) saveBtn.textContent = 'UPDATE ESPORTS';

    esportsForm.scrollIntoView({ behavior: 'smooth' });
};
...
if (esportsForm) {
    esportsForm.addEventListener('submit', async (e) => {
        ...
        const title = document.getElementById('esports-title').value;
        const url = document.getElementById('esports-url').value;
        const link = document.getElementById('esports-link').value;
        const date = document.getElementById('esports-posting-date').value;
        ...
        const rpcPayload = {
            p_email: email,
            p_key: key,
            p_id: isEditingEsports ? parseInt(currentEditEsportsId) : null,
            p_title: title,
            p_url: url,
            p_banner_link: link,
            p_nation_key: 'esports',
            p_start_date: date,
            p_end_date: null // Esports news doesn't have end date usually
        };
...
function resetEsportsForm() {
    isEditingEsports = false;
    currentEditEsportsId = null;
    const formHeader = esportsForm.previousElementSibling.querySelector('h3');
    if (formHeader) formHeader.textContent = 'ADD ESPORTS NEWS';
    esportsForm.reset();
    const saveBtn = document.getElementById('btn-save-esports');
    if (saveBtn) saveBtn.textContent = 'ADD ESPORTS';
}

function updateEsportsCheckboxListeners() {
    const checkboxes = document.querySelectorAll('.esports-checkbox');
    checkboxes.forEach(cb => {
        cb.addEventListener('change', updateEsportsDeleteButtonVisibility);
    });
}

function updateEsportsDeleteButtonVisibility() {
    const checkboxes = document.querySelectorAll('.esports-checkbox');
    const checkedCount = Array.from(checkboxes).filter(cb => cb.checked).length;

    if (btnDeleteEsportsSelected) {
        btnDeleteEsportsSelected.style.display = checkedCount > 0 ? 'inline-flex' : 'none';
    }

    if (selectAllEsports) {
        selectAllEsports.checked = checkboxes.length > 0 && checkedCount === checkboxes.length;
        selectAllEsports.indeterminate = checkedCount > 0 && checkedCount < checkboxes.length;
    }
}

if (selectAllEsports) {
    selectAllEsports.addEventListener('change', function () {
        const checkboxes = document.querySelectorAll('.esports-checkbox');
        checkboxes.forEach(cb => {
            cb.checked = this.checked;
        });
        updateEsportsDeleteButtonVisibility();
    });
}

if (btnDeleteEsportsSelected) {
    btnDeleteEsportsSelected.addEventListener('click', async function () {
        const checkboxes = document.querySelectorAll('.esports-checkbox:checked');
        const ids = Array.from(checkboxes).map(cb => cb.dataset.id);

        if (ids.length === 0) return;

        if (confirm(`Delete ${ids.length} selected esports items?`)) {
            const email = localStorage.getItem('admin_email');
            const key = localStorage.getItem('admin_key');
            ...
            try {
                ...
                console.error('Error deleting esports:', error);
                alert('Error deleting: ' + error.message);
            }
        }
    });
...
if (filterEsportsFrom) filterEsportsFrom.addEventListener('input', filterAndRenderEsports);
if (filterEsportsTo) filterEsportsTo.addEventListener('input', filterAndRenderEsports);
if (btnApplyEsportsFilter) btnApplyEsportsFilter.addEventListener('click', filterAndRenderEsports);
