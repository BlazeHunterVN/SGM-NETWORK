    ...
    try {
        ...
        switch (action) {
            ...
            case 'fetch_admin_list':
                // FETCH email and role ONLY
                result = await supabase.from('admin_access').select('email, role').order('role', { ascending: false }).order('email', { ascending: true });
                break;
        ...
        if (result.error) {
            ...
            return res.status(500).json({ error: result.error.message || 'Operation failed. Please try again.' });
        }
